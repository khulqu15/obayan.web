    ...
    >
      <div class="rounded-[2rem] border border-red-200 bg-white p-8 shadow-sm">
        ...
        <p class="mx-auto mt-2 max-w-md text-sm font-medium leading-6 text-neutral-500">
          Data fasilitas yang kamu buka belum tersedia, sudah tidak aktif, atau slug-nya tidak sesuai.
        </p>
    ...
    <template v-else>
      ...
      <section class="relative overflow-hidden bg-neutral-950 pt-24 text-white">
        <div class="absolute inset-0">
          ...
          >
            <Icon
              :icon="facility.icon || getFacilityTypeIcon(facility.facilityType)"
        ...
        <div class="relative mx-auto max-w-7xl px-4 pb-12 pt-14 sm:px-6 lg:px-8">
          <nav class="mb-8 flex flex-wrap items-center gap-2 text-sm font-bold text-white/60">
            ...
            <NuxtLink to="/facilities" class="transition hover:text-white">
              Fasilitas
            </NuxtLink>
            <Icon icon="solar:alt-arrow-right-linear" class="h-4 w-4" />
            ...
            <aside class="rounded-[2rem] border border-white/15 bg-white/10 p-5 shadow-2xl shadow-neutral-950/20 backdrop-blur-xl">
              <div class="flex items-start gap-4">
                <div class="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-3xl bg-white/10 text-white">
                  <img
                    v-if="facility.logoUrl && !logoFailed"
              ...
              <div class="mt-5 grid gap-2">
                <InfoRow label="Kategori" :value="facilityTypeLabel(facility.facilityType)" />
                <InfoRow label="Urutan" :value="String(facility.sortOrder || 0)" />
                <InfoRow label="Update" :value="formatDate(facility.updatedAt)" />
              </div>
            </aside>
          </div>
        </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-sm font-black text-neutral-950">
                  Detail {{ facility.title }}
                </p>
                <p class="text-xs font-semibold text-neutral-500">
                  Informasi lengkap fasilitas desa
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="px-5 py-6 sm:px-6">
            ...
            >
              ...
              <p class="mt-3 text-sm font-bold text-neutral-500">
                Belum ada content detail untuk fasilitas ini.
              </p>
            </div>
          </div>
        </article>
        ...
        <aside class="space-y-4">
          <div class="rounded-[2rem] border border-neutral-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 space-y-4">
              ...
              <div v-if="facility.phone || facility.whatsapp">
                <p class="text-xs font-black uppercase tracking-[0.14em] text-neutral-400">
                  Kontak
                </p>
                <p class="mt-1 text-sm font-bold text-neutral-700">
                  {{ facility.whatsapp || facility.phone }}
                </p>
              </div>

              <div v-if="facility.email">
                <p class="text-xs font-black uppercase tracking-[0.14em] text-neutral-400">
                  Email
                </p>
                <a
                  :href="`mailto:${facility.email}`"
                  class="mt-1 block truncate text-sm font-bold text-blue-600 hover:underline"
                >
                  {{ facility.email }}
                </a>
              </div>

              <div v-if="facility.websiteUrl">
                <p class="text-xs font-black uppercase tracking-[0.14em] text-neutral-400">
                  Website
                </p>
                <a
        ...
        <div class="mb-5 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 class="mt-2 text-2xl font-black tracking-tight text-neutral-950">
              Fasilitas lain yang mungkin relevan
            </h2>
          </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div class="relative h-40 bg-neutral-100">
              <img
                v-if="getFacilityImage(item) && !relatedFailedImages[item.id]"
            ...
            <div class="p-5">
              <p class="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">
                <Icon :icon="getFacilityTypeIcon(item.facilityType)" class="h-3.5 w-3.5" />
                {{ facilityTypeLabel(item.facilityType) }}
              </p>

              <h3 class="mt-3 line-clamp-2 text-lg font-black leading-7 text-neutral-950 group-hover:text-blue-700">
                {{ item.title }}
              </h3>

              <p class="mt-2 line-clamp-2 text-sm font-medium leading-6 text-neutral-500">
                {{ item.subtitle || 'Informasi fasilitas desa.' }}
              </p>
            </div>
          </NuxtLink>
        </div>
...
const {
  ...
  pending,
  error
} = await useFetch<FacilityDetailResponse>(apiUrl, {
  key: computed(() => `facility-detail-${tenantSlug.value}-${slug.value}`),
...
const facility = computed<FacilityItem | null>(() => {
...
} = await useFetch<FacilityListResponse>(relatedApiUrl, {
  key: computed(() => `facility-related-${tenantSlug.value}-${facility.value?.facilityType || 'all'}`),
  query: computed(() => ({
    status: 'active',
    type: facility.value?.facilityType,
    limit: 6,
    sort: 'sort_order'
  })),
  watch: [tenantSlug, facility],
  default: () => ({
    data: [],
    meta: {
      page: 1,
...
const heroImage = computed(() => {
  ...
  return facility.value.imageUrl || facility.value.logoUrl || ''
})

const safeContentHtml = computed(() => {
  return sanitizeHtml(facility.value?.contentHtml || '')
})

const whatsappUrl = computed(() => {
  const value = facility.value?.whatsapp || facility.value?.phone || ''
...
const mapsUrl = computed(() => {
  ...
  if (facility.value.latitude && facility.value.longitude) {
    return `https://www.google.com/maps/search/?api=1&query=${facility.value.latitude},${facility.value.longitude}`
  }

  if (facility.value.address) {
    return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(facility.value.address)}`
  }
...
const relatedFacilities = computed(() => {
  const currentId = facility.value?.id
...
const pageTitle = computed(() => {
  return facility.value
    ? `${facility.value.title} · Fasilitas Desa`
    : 'Facility tidak ditemukan'
})

const pageDescription = computed(() => {
  return facility.value?.subtitle ||
    plainText(facility.value?.contentHtml || '').slice(0, 160) ||
...
useHead(() => ({
  ...
  link: [
    {
      rel: 'canonical',
      href: `${requestUrl.origin}/facilities/${slug.value}`
    }
  ]
}))
...
const facilityTypeOptions: {
  ...
  { value: 'custom', label: 'Fasilitas', icon: 'lucide:layers' }
]
