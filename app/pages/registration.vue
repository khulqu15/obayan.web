            ...
            <div class="p-5 sm:p-7">
                ...
                <div class="relative rounded-3xl border border-emerald-200/60 dark:border-emerald-900/40 bg-white/85 dark:bg-neutral-900/80 backdrop-blur-xl shadow-[0_14px_60px_rgba(16,185,129,.18)]">
                  ...
                  <div class="p-6 sm:p-8">
                    <div class="mb-4 inline-flex items-center gap-3">
                      ...
                      <h2 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Informasi Penting Sebelum Mengisi Formulir</h2>
                    </div>
                    ...
                    <div class="prose prose-sm sm:prose-base dark:prose-invert max-w-none">
                      ...
                      <p>Formulir ini digunakan untuk pendaftaran resmi santri baru lulusan SD/MI. Mohon diisi dengan jujur, lengkap, dan sesuai dokumen resmi (Akta Kelahiran, Kartu Keluarga, KTP Ayah & Ibu).</p>
                      <p>Data yang tercantum akan dipakai sebagai arsip administrasi pondok pesantren. Mohon membaca petunjuk sebelum mengisi.</p>
              ...
              <div v-else-if="step===1" class="grid sm:grid-cols-2 gap-4">
                <label class="block">
                  ...
                  <input v-model="form.siswa.nama" type="text" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Jenis Kelamin <span class="text-red-600">*</span></span>
                  <select v-model="form.siswa.jk" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required>
                ...
                <label class="block">
                  ...
                  <input v-model="form.siswa.tmpLahir" type="text" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Tanggal Lahir <span class="text-red-600">*</span></span>
                  <input v-model="form.siswa.tglLahir" type="date" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Tinggi Badan (cm)</span>
                  <input v-model="form.siswa.tinggi" type="number" min="0" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Berat Badan (kg)</span>
                  <input v-model="form.siswa.berat" type="number" min="0" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Cita-cita</span>
                  <input v-model="form.siswa.cita" type="text" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" placeholder="Contoh: Dokter" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Hobi</span>
                  <input v-model="form.siswa.hobi" type="text" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" placeholder="Contoh: Membaca" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">No KK <span class="text-red-600">*</span></span>
                  <input v-model="form.siswa.kk" type="text" inputmode="numeric" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">No NIK <span class="text-red-600">*</span></span>
                  <input v-model="form.siswa.nik" type="text" inputmode="numeric" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block sm:col-span-2">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">NISN <span class="text-red-600">*</span></span>
                  <input v-model="form.siswa.nisn" type="text" inputmode="numeric" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
              </div>
              ...
              <div v-else-if="step===2" class="grid sm:grid-cols-2 gap-4">
                <label class="block">
                  ...
                  <input v-model="form.alamat.jalan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <div class="grid grid-cols-2 gap-3">
                  <label class="block">
                    <span class="text-xs text-gray-600 dark:text-neutral-300">RT</span>
                    <input v-model="form.alamat.rt" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                  </label>
                  <label class="block">
                    <span class="text-xs text-gray-600 dark:text-neutral-300">RW</span>
                    <input v-model="form.alamat.rw" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                  </label>
                </div>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Dusun</span>
                  <input v-model="form.alamat.dusun" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Desa/Kelurahan <span class="text-red-600">*</span></span>
                  <input v-model="form.alamat.desa" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Kecamatan <span class="text-red-600">*</span></span>
                  <input v-model="form.alamat.kec" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Kabupaten/Kota <span class="text-red-600">*</span></span>
                  <input v-model="form.alamat.kab" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Provinsi <span class="text-red-600">*</span></span>
                  <input v-model="form.alamat.prov" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Kode Pos</span>
                  <input v-model="form.alamat.kodepos" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
              ...
              <!-- STEP 3: RIWAYAT PENDIDIKAN (SD/MI Only) -->
              <div v-else-if="step===3" class="grid sm:grid-cols-2 gap-4">
                ...
                <label class="block">
                  ...
                  <input v-model="form.pendidikan.sekolah" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" placeholder="SD/MI" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Alamat Sekolah Asal</span>
                  <input v-model="form.pendidikan.alamatSekolah" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                ...
                <label class="block">
                  ...
                  <input v-model="form.pendidikan.nfNama" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Alamat Lembaga Non-Formal</span>
                  <input v-model="form.pendidikan.nfAlamat" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
              </div>
              ...
              <div v-else-if="step===4" class="grid sm:grid-cols-2 gap-4">
                ...
                <label class="block">
                  ...
                  <input v-model="form.ortu.ayah.nama" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Status</span>
                  <select v-model="form.ortu.ayah.status" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Hidup</option>
                ...
                <label class="block">
                  ...
                  <select v-model="form.ortu.ayah.pendidikan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                ...
                <label class="block">
                  ...
                  <select v-model="form.ortu.ayah.pekerjaan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                ...
                <label class="block">
                  ...
                  <select v-model="form.ortu.ayah.penghasilan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                ...
                <label class="block">
                  ...
                  <input v-model="form.ortu.ibu.nama" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Status</span>
                  <select v-model="form.ortu.ibu.status" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Hidup</option>
                    <option>Meninggal</option>
                  </select>
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Pendidikan Terakhir</span>
                  <select v-model="form.ortu.ibu.pendidikan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Tidak Sekolah</option><option>SD</option><option>SMP</option><option>SMA</option><option>D1/D2</option><option>S1</option><option>S2</option><option>S3</option>
                  </select>
                </label>
                <!-- Kontak -->
                <label class="block sm:col-span-2">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Nomor HP 1 (WA aktif) <span class="text-red-600">*</span></span>
                  <input v-model="form.ortu.hp1" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" required placeholder="*08xxxxxxxxxx" />
                </label>
                <label class="block sm:col-span-2">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Nomor HP 2 (Opsional)</span>
                  <input v-model="form.ortu.hp2" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                ...
                <label class="block">
                  ...
                  <input v-model="form.wali.nama" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Status</span>
                  <select v-model="form.wali.status" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Hidup</option>
                    <option>Meninggal</option>
                  </select>
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Pendidikan Terakhir</span>
                  <select v-model="form.wali.pendidikan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Tidak Sekolah</option><option>SD</option><option>SMP</option><option>SMA</option><option>D1/D2</option><option>S1</option><option>S2</option><option>S3</option>
                  </select>
                </label>
                <label class="block">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Pekerjaan</span>
                  <select v-model="form.wali.pekerjaan" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm">
                    <option>Tidak Bekerja</option><option>Petani</option><option>PNS/ASN</option><option>TNI/POLRI</option><option>Guru</option><option>Swasta</option><option>Wiraswasta</option><option>Pengusaha</option><option>Peternak</option><option>Lainnya</option>
                  </select>
                </label>
                <label class="block sm:col-span-2">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Nomor HP Wali (Opsional)</span>
                  <input v-model="form.wali.hp" class="w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white/90 dark:bg-neutral-900 px-4 py-2.5 text-sm" />
                </label>
              </div>

              <!-- STEP 5: DOKUMEN (4 jenis) — TANPA KOMPONEN BARU -->
              <div v-else-if="step===5" class="grid sm:grid-cols-2 gap-4">
                <!-- Tile: KK -->
                <div class="space-y-1.5">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Kartu Keluarga (PDF/JPG) *</span>
                  <div
                    ...
                    @dragover.prevent
                    @drop.prevent="handleDrop('kk', $event)"
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      <div class="shrink-0 rounded-md bg-gray-100 dark:bg-neutral-800 p-2 text-gray-600 dark:text-neutral-300">📎</div>
                      <div class="min-w-0">
                        <p class="font-medium truncate">{{ files.kk.file?.name || 'Tarik & lepas file ke sini' }}</p>
                        <p class="text-xs text-gray-500 dark:text-neutral-400">
                          {{ fileSizeText(files.kk.file?.size) || 'Maks 2MB' }}
                        </p>
                      </div>
                    </div>
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white/80 dark:bg-neutral-800/60 px-3 py-2 text-sm font-medium hover:bg-white dark:hover:bg-neutral-800">
                      <input type="file" class="hidden" accept=".pdf,image/*" @change="onFileChange('kk', $event)" />
                      Pilih File
                    </label>
                  </div>
                ...
                <!-- Tile: Akta -->
                <div class="space-y-1.5">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">Akte Kelahiran (PDF/JPG) *</span>
                  <div
                    class="relative rounded-xl border border-dashed border-gray-300 dark:border-neutral-700 p-4 bg-white/70 dark:bg-neutral-900/60 text-sm flex items-center justify-between gap-3"
                    @dragover.prevent
                    @drop.prevent="handleDrop('akte', $event)"
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      <div class="shrink-0 rounded-md bg-gray-100 dark:bg-neutral-800 p-2 text-gray-600 dark:text-neutral-300">📎</div>
                      <div class="min-w-0">
                        <p class="font-medium truncate">{{ files.akte.file?.name || 'Tarik & lepas file ke sini' }}</p>
                        <p class="text-xs text-gray-500 dark:text-neutral-400">
                          {{ fileSizeText(files.akte.file?.size) || 'Maks 2MB' }}
                        </p>
                      </div>
                    </div>
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white/80 dark:bg-neutral-800/60 px-3 py-2 text-sm font-medium hover:bg-white dark:hover:bg-neutral-800">
                      <input type="file" class="hidden" accept=".pdf,image/*" @change="onFileChange('akte', $event)" />
                      Pilih File
                    </label>
                  </div>
                ...
                <!-- Tile: KTP Ayah -->
                <div class="space-y-1.5">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">KTP Ayah (PDF/JPG) *</span>
                  <div
                    class="relative rounded-xl border border-dashed border-gray-300 dark:border-neutral-700 p-4 bg-white/70 dark:bg-neutral-900/60 text-sm flex items-center justify-between gap-3"
                    @dragover.prevent
                    @drop.prevent="handleDrop('ktpAyah', $event)"
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      <div class="shrink-0 rounded-md bg-gray-100 dark:bg-neutral-800 p-2 text-gray-600 dark:text-neutral-300">📎</div>
                      <div class="min-w-0">
                        <p class="font-medium truncate">{{ files.ktpAyah.file?.name || 'Tarik & lepas file ke sini' }}</p>
                        <p class="text-xs text-gray-500 dark:text-neutral-400">
                          {{ fileSizeText(files.ktpAyah.file?.size) || 'Maks 2MB' }}
                        </p>
                      </div>
                    </div>
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white/80 dark:bg-neutral-800/60 px-3 py-2 text-sm font-medium hover:bg-white dark:hover:bg-neutral-800">
                      <input type="file" class="hidden" accept=".pdf,image/*" @change="onFileChange('ktpAyah', $event)" />
                      Pilih File
                    </label>
                  </div>
                ...
                <!-- Tile: KTP Ibu -->
                <div class="space-y-1.5">
                  <span class="text-xs text-gray-600 dark:text-neutral-300">KTP Ibu (PDF/JPG) *</span>
                  <div
                    class="relative rounded-xl border border-dashed border-gray-300 dark:border-neutral-700 p-4 bg-white/70 dark:bg-neutral-900/60 text-sm flex items-center justify-between gap-3"
                    @dragover.prevent
                    @drop.prevent="handleDrop('ktpIbu', $event)"
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      <div class="shrink-0 rounded-md bg-gray-100 dark:bg-neutral-800 p-2 text-gray-600 dark:text-neutral-300">📎</div>
                      <div class="min-w-0">
                        <p class="font-medium truncate">{{ files.ktpIbu.file?.name || 'Tarik & lepas file ke sini' }}</p>
                        <p class="text-xs text-gray-500 dark:text-neutral-400">
                          {{ fileSizeText(files.ktpIbu.file?.size) || 'Maks 2MB' }}
                        </p>
                      </div>
                    </div>
                    <label class="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white/80 dark:bg-neutral-800/60 px-3 py-2 text-sm font-medium hover:bg-white dark:hover:bg-neutral-800">
                      <input type="file" class="hidden" accept=".pdf,image/*" @change="onFileChange('ktpIbu', $event)" />
                      Pilih File
                    </label>
                  </div>
              ...
              <div v-else-if="step===6" class="space-y-4">
                <div class="rounded-xl border border-gray-200 dark:border-neutral-800 p-4">
                  <div class="flex items-center gap-2 text-sm font-semibold">
                    <Icon icon="ph:check-circle" class="size-5 600 dark:300" />
                  ...
                  <div class="grid sm:grid-cols-2 gap-3 text-sm">
                    <div class="flex items-center justify-between gap-3">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button @click="copy(receipt.username)" class="px-2 py-1 rounded bg-gray-100 dark:bg-neutral-800">Copy</button>
                      </div>
                    </div>
                    <div class="flex items-center justify-between gap-3">
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button @click="copy(receipt.publicToken)" class="px-2 py-1 rounded bg-gray-100 dark:bg-neutral-800">Copy</button>
                      </div>
                    </div>
                  </div>
        ...
        <aside class="lg:col-span-4">
          <div class="space-y-4">
            <div class="rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-neutral-900/60 backdrop-blur p-5 shadow">
              <div class="flex items-center gap-2">
                <Icon icon="ph:list-checks" class="size-5 600 dark:300" />
                <h3 class="font-semibold">Syarat Umum & Petunjuk</h3>
              </div>
              <ul class="mt-3 text-sm space-y-2">
                <li>Siapkan: Akta Kelahiran, KK, KTP Ayah, KTP Ibu.</li>
                <li>Masukkan nomor (NIK/KK/NISN/HP) sesuai dokumen.</li>
                <li>Gunakan huruf kapital seperlunya (nama, alamat).</li>
              </ul>
            </div>

            <div class="rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-neutral-900/60 backdrop-blur p-5 shadow">
              <div class="flex items-center gap-2">
                <Icon icon="ph:calendar-check" class="size-5 600 dark:300" />
            ...
            <div class="rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-neutral-900/60 backdrop-blur p-5 shadow">
              <div class="flex items-center gap-2">
                <Icon icon="ph:headset" class="size-5 600 dark:300" />
          ...
          <div class="relative flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
            <div class="absolute top-2 end-2">
              <button type="button"
                ...
                aria-label="Close" @click="brochureOpen = false">
            ...
            <div class="px-5 sm:px-8 pt-6 pb-3 border-b border-gray-200 dark:border-neutral-800">
              <h3 class="text-lg font-semibold text-gray-800 dark:text-neutral-200">Brosur PPDB</h3>
              <p class="text-xs text-gray-500 dark:text-neutral-400">Gunakan <code>CTRL</code> untuk zoom. Seret untuk geser.</p>
            </div>

            <div v-if="ppdbBrochureImages.length" class="relative w-full aspect-video rounded-xl border border-gray-200 overflow-hidden bg-gray-100 dark:border-neutral-800 dark:bg-neutral-800 select-none">
              <div
                ref="ppdbStageRef"
              ...
              >
                <img :src="ppdbBrochureImages[ppdbIdx]" :alt="`Brosur ${ppdbIdx + 1}`" class="absolute top-1/2 left-1/2 will-change-transform" :style="ppdbImgStyle" draggable="false" loading="eager" />
              </div>
            ...
            <div class="flex flex-wrap justify-between items-center gap-2 py-3 px-4 bg-gray-50 border-t border-gray-200 dark:bg-neutral-950 dark:border-neutral-800">
              <div class="text-xs text-gray-500 dark:text-neutral-400">Total: {{ ppdbBrochureImages.length }} file</div>
              <div class="flex gap-2">
                <a :href="ppdbBrochureImages[ppdbIdx]" download class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-700">
                  <Icon icon="lucide:download" class="size-4" /> Unduh
                </a>
                <button type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden dark:bg-neutral-800 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-700" @click="brochureOpen=false">
                  Tutup
                </button>
              </div>
            </div>
...
const files = reactive<Record<'kk'|'akte'|'ktpAyah'|'ktpIbu', FileBox>>({
  kk:{file:null,url:null}, akte:{file:null,url:null}, ktpAyah:{file:null,url:null}, ktpIbu:{file:null,url:null}
})
...
function validStep(i: number) {
  switch (i) {
    case 0: return true
    case 1: return !!(form.siswa.nama && form.siswa.jk && form.siswa.tmpLahir && form.siswa.tglLahir && form.siswa.nik)
    case 2: return !!(form.alamat.jalan && form.alamat.desa && form.alamat.kec && form.alamat.kab && form.alamat.prov)
    case 3: return true // SD/MI only — tidak wajib isi semua
    case 4: return !!(form.ortu.ayah.nama && form.ortu.ibu.nama && form.ortu.hp1)
    case 5: return !!(files.kk.file && files.akte.file && files.ktpAyah.file && files.ktpIbu.file && form.pernyataan.keaslian)
    case 6: return true
...
const dokumenCheck = computed(() => {
  const ok = ['kk','akte','ktpAyah','ktpIbu'].every(k => (files as any)[k]?.file)
  return ok ? 'Lengkap' : 'Belum Lengkap'
})

/** ===== Upload handlers (tanpa komponen baru) ===== */
function onFileChange(key:'kk'|'akte'|'ktpAyah'|'ktpIbu', e: Event) {
  const input = e.target as HTMLInputElement
  const f = input.files?.[0] || null
  const url = f ? URL.createObjectURL(f) : null
  ;(files as any)[key] = { file: f, url }
}
function handleDrop(key:'kk'|'akte'|'ktpAyah'|'ktpIbu', e: DragEvent) {
...
function validateFileSizes(): string | null {
  const lim = { kk: 2*1024*1024, akte: 2*1024*1024, ktpAyah: 2*1024*1024, ktpIbu: 2*1024*1024 }
  for (const k of Object.keys(lim) as Array<keyof typeof lim>) {
    const f = (files as any)[k]?.file
    if (!f) return 'Mohon lengkapi semua dokumen.'
    if (f.size > lim[k]) return `Ukuran ${String(k).toUpperCase()} melebihi batas.`
  }
  return null
}
...
/** ===== PPDB realtime settings (open/close) ===== */
...
function toMs(v:any): number | null {
  ...
  if (typeof v === 'string') {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(s)) s += 'T23:59:59+07:00'
    else if (/^\d{4}-\d{2}-\d{2}\s+\d/.test(s)) s = s.replace(' ', 'T')
    if (!/[zZ]|[+-]\d{2}:\d{2}$/.test(s)) s += ' +07:00'
    const ms = Date.parse(s); return Number.isNaN(ms) ? null : ms
  }
...
const closedReason = computed(() => {
  ...
  if (isAutoDue.value && autoAtMs.value) return `Pendaftaran ditutup otomatis pada ${formatInJktMs(autoAtMs.value.toString())}.`
...
watch(brochureOpen, (v) => { document.body.style.overflow = v ? 'hidden' : '' })
...
const countdownText = computed(() => {
  ...
  const sec = Math.floor(t / 1000), d = Math.floor(sec / 86400)
...
async function submit(){
  ...
  loading.value = true; feedback.value = ''; ok.value = false; regInfo.value = null; receipt.value = null
  ...
  try {
    ...
    const up = async (key:'kk'|'akte'|'ktpAyah'|'ktpIbu') => {
      ...
      const r = sRef(store, storagePath); await uploadBytes(r, f)
      return await getDownloadURL(r)
    }
    const [kkUrl, akteUrl, ktpAyahUrl, ktpIbuUrl] = await Promise.all([ up('kk'), up('akte'), up('ktpAyah'), up('ktpIbu') ])
...
const ppdbImgStyle = computed(() => ({ transform: `translate(-50%, -50%) translate(${ppdbPos.x}px, ${ppdbPos.y}px) scale(${ppdbScale.value})` }))
function ppdbReset(){ ppdbScale.value = 1; ppdbPos.x = 0; ppdbPos.y = 0 }
function ppdbZoomIn(){ ppdbScale.value = Math.min(5, ppdbScale.value + 0.1) }
function ppdbZoomOut(){ ppdbScale.value = Math.max(0.2, ppdbScale.value - 0.1) }
function ppdbPrev(){ if (ppdbIdx.value > 0) { ppdbIdx.value--; ppdbReset() } }
function ppdbNext(){ if (ppdbIdx.value < ppdbBrochureImages.value.length - 1) { ppdbIdx.value++; ppdbReset() } }
function ppdbOnWheel(e: WheelEvent){ const delta = -Math.sign(e.deltaY) * 0.05; const next = ppdbScale.value + delta; ppdbScale.value = Math.min(5, Math.max(0.2, next)) }
function ppdbOnPointerDown(e: MouseEvent){ ppdbDragging.value = true; ;(ppdbStageRef.value as HTMLElement).style.cursor = 'grabbing'; (ppdbStageRef.value as HTMLElement).dataset.dx = String(e.clientX - ppdbPos.x); (ppdbStageRef.value as HTMLElement).dataset.dy = String(e.clientY - ppdbPos.y) }
function ppdbOnPointerMove(e: MouseEvent){ if(!ppdbDragging.value) return; const dx = Number((ppdbStageRef.value as HTMLElement).dataset.dx || 0); const dy = Number((ppdbStageRef.value as HTMLElement).dataset.dy || 0); ppdbPos.x = e.clientX - dx; ppdbPos.y = e.clientY - dy }
function ppdbOnPointerUp(){ ppdbDragging.value = false; if (ppdbStageRef.value) (ppdbStageRef.value as HTMLElement).style.cursor = '' }
function ppdbOnTouchStart(e: TouchEvent){ if(e.touches.length===1){ (ppdbStageRef.value as HTMLElement).dataset.txx = String(e.touches[0]!.clientX - ppdbPos.x); (ppdbStageRef.value as HTMLElement).dataset.tyy = String(e.touches[0]!.clientY - ppdbPos.y) } }
function ppdbOnTouchMove(e: TouchEvent){ if(e.touches.length===1){ const t=e.touches[0]; const dx = Number((ppdbStageRef.value as HTMLElement).dataset.txx || 0); const dy = Number((ppdbStageRef.value as HTMLElement).dataset.tyy || 0); ppdbPos.x = t!.clientX - dx; ppdbPos.y = t!.clientY - dy } }
function ppdbOnTouchEnd(){ /* noop */ }
function ppdbOnDblClick(){ ppdbReset() }
