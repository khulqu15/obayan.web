...
<template>
  <div class="relative">
    ...
    <div class="p-6 space-y-6">
      ...
      <!-- SETTINGS: Buka/Tutup (Duolingo-ish) -->
      <section
        class="rounded-2xl border border-slate-200/70 dark:border-neutral-800 bg-white/85 dark:bg-neutral-900/80 backdrop-blur p-4 sm:p-5">
          ...
          <div v-if="settings.autoCloseEnabled" class="grid md:grid-cols-[1fr,auto] gap-3">
            <div class="grid sm:grid-cols-2 gap-3">
              ...
              <div class="space-y-1">
                ...
                <div v-if="willAutoClose" class="text-[11px] text-emerald-700 dark:text-emerald-300">
...
/** Sync tahun dari settings + fallback ke data */
onMounted(async () => {
...
/** Debounce kecil untuk saveSettings(year) */
...
/** Tahun gabungan: dari data + settings agar opsi tahun settings tetap muncul */
...
/** AutoClose datetime-local binding */
const autoCloseLocal = computed(() => {
  const iso = settings.value.autoCloseAt
  if (!iso) return ''
...
let timer: any = null
onMounted(() => {
  timer = setInterval(() => { tick.value = Date.now(); checkAutoClose() }, 30000) // 30s
})
...
/** Auto-close worker (client) */
async function checkAutoClose() {
  if (!settings.value.autoCloseEnabled || !settings.value.autoCloseAt) return
  const due = new Date(settings.value.autoCloseAt).getTime()
  if (Date.now() >= due && !settings.value.isClosed) {
    await saveSettings({ isClosed: true, autoCloseEnabled: false })
  }
}

/** Quick picks */
function atEndOfToday(): Date {
  const d = new Date()
...
function testScheduleNow(){
  const d = new Date(Date.now()+60_000) // 1 menit ke depan
  saveSettings({ autoCloseAt: d.toISOString(), autoCloseEnabled: true })
}

/** ===== Helper ===== */
const formUrlComputed = computed(() => formUrl) // kept for copy button if needed
