        ...
        >
          <div class="relative h-52 bg-neutral-100">
            ...
            <div v-else class="flex h-full w-full items-center justify-center bg-neutral-100 text-neutral-400">
              <Icon :icon="item.icon || defaultIconByType(item.potentialType)" class="h-10 w-10" />
            </div>
            ...
            <div class="absolute left-3 top-3 flex flex-wrap gap-2">
              ...
              <span v-if="item.isFeatured" class="rounded-full bg-emerald-600 px-3 py-1 text-xs font-black text-white backdrop-blur">
                Featured
              </span>
            </div>

            <span
              v-if="getPotentialImages(item).length > 1"
              class="absolute bottom-3 right-3 rounded-full bg-neutral-950/80 px-3 py-1 text-xs font-black text-white backdrop-blur"
            >
              {{ getPotentialImages(item).length }} Foto
            </span>
          </div>
          ...
          <div class="p-5">
            ...
            <div class="mt-3 inline-flex items-center gap-2 rounded-full bg-[var(--brand-soft)] px-3 py-1 text-xs font-black text-[var(--brand)]">
              <Icon :icon="item.icon || defaultIconByType(item.potentialType)" class="h-4 w-4" />
              {{ potentialTypeLabel(item.potentialType) }}
            </div>
            ...
            <div class="mt-4 grid gap-2 text-xs font-semibold text-neutral-500">
              <div v-if="item.productionCapacity || item.productionUnit" class="flex items-center gap-2">
                <Icon icon="solar:chart-bold-duotone" class="h-4 w-4 text-neutral-400" />
              ...
              <div v-if="item.marketReach" class="flex items-center gap-2">
                <Icon icon="solar:shop-bold-duotone" class="h-4 w-4 text-neutral-400" />
                <span class="truncate">{{ item.marketReach }}</span>
              </div>

              <div v-if="item.ownerName || item.managerName" class="flex items-center gap-2">
                <Icon icon="solar:user-id-bold-duotone" class="h-4 w-4 text-neutral-400" />
                <span class="truncate">{{ item.ownerName || item.managerName }}</span>
              </div>
            </div>
                ...
                <div class="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <h2 class="text-xl font-black text-neutral-950">
                      {{ formMode === 'create' ? profile.createLabel : 'Edit Potensi' }}
                    </h2>
                  </div>
                  ...
                  <div class="rounded-[1.75rem] border border-neutral-200 bg-neutral-50 p-5">
                    <div class="flex items-start gap-4">
                      <div class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white text-[var(--brand)] shadow-sm">
                        <Icon icon="solar:leaf-bold-duotone" class="h-5 w-5" />
                      </div>

                      <div>
                        <h3 class="text-base font-black text-neutral-950">Identitas Potensi</h3>
                        <p class="mt-1 text-sm font-medium leading-6 text-neutral-500">
                          Isi nama, slug, tipe potensi, icon, status, featured, dan urutan tampil.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="grid gap-4 lg:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        type="text"
                        required
                        placeholder="Contoh: UMKM Keripik Singkong Martopuro"
                        class="input-field"
                    ...
                    <div>
                      ...
                      <input
                        ...
                        type="text"
                        required
                        placeholder="umkm-keripik-singkong-martopuro"
                        class="input-field"
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Icon Potensi</label>
                      ...
                      >
                        <span class="flex min-w-0 items-center gap-3">
                          <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white text-[var(--brand)] shadow-sm">
                            <Icon :icon="form.icon || defaultIconByType(form.potentialType)" class="h-6 w-6" />
                          </span>
                    ...
                    <div class="lg:col-span-2">
                      <label class="inline-flex cursor-pointer items-center gap-3 rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3">
                        ...
                        <span>
                          ...
                          <span class="block text-xs font-semibold text-neutral-500">Potensi akan diprioritaskan di section public website.</span>
                        </span>
                      </label>
                    </div>
                  </div>
                ...
                <div v-if="formStep === 3" class="space-y-5">
                  <div class="rounded-[1.75rem] border border-neutral-200 bg-neutral-50 p-5">
                    <div class="flex items-start gap-4">
                      ...
                      <div>
                        <h3 class="text-base font-black text-neutral-950">Detail, Produksi, Lokasi & Kontak</h3>
                        <p class="mt-1 text-sm font-medium leading-6 text-neutral-500">
                          Isi ringkasan, kapasitas produksi, pengelola, jangkauan pasar, kontak, lokasi, dan deskripsi lengkap menggunakan Tiptap editor.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="grid gap-4 lg:grid-cols-2">
                    ...
                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Owner / Pemilik</label>
                      <input v-model.trim="form.ownerName" type="text" placeholder="Nama pemilik / kelompok" class="input-field">
                    </div>

                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Manager / Pengelola</label>
                    ...
                    <div>
                      ...
                      <input v-model.trim="form.productionCapacity" type="text" placeholder="Contoh: 100" class="input-field">
                    </div>

                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Satuan Produksi</label>
                      <input v-model.trim="form.productionUnit" type="text" placeholder="Kg/bulan, Unit, Ton, Produk" class="input-field">
                    </div>

                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Jangkauan Pasar</label>
                      <input v-model.trim="form.marketReach" type="text" placeholder="Pasar lokal, kecamatan, online, regional" class="input-field">
                    </div>

                    <div>
                      <label class="mb-2 block text-sm font-black text-neutral-700">Estimasi Nilai</label>
                      <input v-model.trim="form.estimatedValue" type="text" placeholder="Contoh: 5000000" class="input-field">
                    </div>
                  ...
                  <div class="rounded-[1.75rem] border border-neutral-200 bg-neutral-50 p-5">
                    <div class="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p class="mt-1 text-sm font-medium leading-6 text-neutral-500">Periksa kembali data potensi sebelum disimpan.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="grid gap-5 lg:grid-cols-[340px_1fr]">
                    ...
                    <div class="rounded-[1.75rem] border border-neutral-200 bg-white p-5">
                      <div class="grid gap-4">
                        ...
                        <ReviewRow label="Produksi" :value="form.productionCapacity || '-'" />
                        <ReviewRow label="Pasar" :value="form.marketReach || '-'" />
                ...
                <div class="mt-6 flex flex-col-reverse gap-3 border-t border-neutral-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div class="flex flex-col-reverse gap-3 sm:flex-row">
                    ...
                    >
                      ...
                      />
                      {{ imageUploading ? 'Upload gambar...' : saving ? 'Menyimpan...' : formMode === 'create' ? 'Simpan Potensi' : 'Update Potensi' }}
                    </button>
                  </div>
                </div>
              </form>
            ...
            <div class="w-full overflow-hidden rounded-[2rem] border border-neutral-200 bg-white shadow-2xl">
              <div class="border-b border-neutral-200 bg-white/95 p-5 backdrop-blur-xl">
                <div class="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h2 class="mt-1 text-xl font-black text-neutral-950">Pilih Icon Potensi</h2>
                    <p class="mt-1 text-sm font-medium leading-6 text-neutral-500">Cari icon dari Iconify API, filter berdasarkan icon set, lalu pilih icon untuk potensi.</p>
                  </div>
                ...
                <div class="mt-5 grid gap-3 lg:grid-cols-[1fr_220px_220px_auto]">
                  <div class="relative">
                    ...
                    <input
                      ...
                      type="text"
                      placeholder="Cari: wheat, store, fish, tourism, industry..."
                      class="input-field py-3 pl-12"
                ...
                <div class="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <span class="inline-flex items-center gap-2 rounded-full bg-[var(--brand-soft)] px-3 py-1 text-xs font-black text-[var(--brand)]">
                    <Icon :icon="form.icon || defaultIconByType(form.potentialType)" class="h-4 w-4" />
                    {{ form.icon || '-' }}
                  </span>
              ...
              <div class="max-h-[62vh] overflow-y-auto p-5">
                ...
                <div v-else-if="iconResults.length === 0" class="rounded-[2rem] border border-dashed border-neutral-300 bg-neutral-50 p-10 text-center">
                  ...
                  <p class="mx-auto mt-2 max-w-md text-sm font-medium leading-6 text-neutral-500">Masukkan keyword pencarian, misalnya <b>wheat</b>, <b>store</b>, <b>fish</b>, <b>industry</b>, atau <b>tourism</b>.</p>
                </div>
      ...
      <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
        <div v-if="showDelete" class="fixed inset-0 z-[9991] flex items-center justify-center bg-neutral-950/50 p-4 backdrop-blur-sm">
          <div class="w-full max-w-md rounded-[2rem] border border-neutral-200 bg-white p-5 shadow-2xl">
            ...
            <h2 class="mt-5 text-xl font-black text-neutral-950">Hapus Potensi?</h2>

            <p class="mt-2 text-sm font-medium leading-6 text-neutral-500">
              Potensi <strong class="text-neutral-900">{{ selectedPotential?.title }}</strong> akan dihapus. Tindakan ini tidak dapat dibatalkan.
            </p>
...
const profile = computed(() => {
  if (isMartopuro.value) {
    return {
      name: 'Desa Martopuro',
      title: 'Potensi Desa Martopuro',
      badge: 'Village Potentials',
      icon: 'solar:leaf-bold-duotone',
      description: 'Kelola potensi unggulan Desa Martopuro seperti pertanian, UMKM, wisata, budaya, peternakan, perikanan, ekonomi kreatif, dan sumber daya lokal lainnya.',
      createLabel: 'Tambah Potensi',
      searchPlaceholder: 'Cari potensi, UMKM, pertanian, wisata, pasar, owner...',
      emptyDescription: 'Belum ada potensi. Tambahkan potensi desa agar dapat ditampilkan di website.',
      brand: '#059669',
      brandSoft: '#ecfdf5',
      brandRing: 'rgba(5, 150, 105, 0.14)'
    }
  }

  return {
    name: 'Obayan',
    title: 'Potentials Obayan',
    badge: 'Platform Potentials',
    icon: 'solar:leaf-bold-duotone',
    description: 'Kelola potensi, produk unggulan, ekonomi lokal, UMKM, budaya, dan aset tenant yang memiliki nilai informasi dan publikasi.',
    createLabel: 'Tambah Potential',
    searchPlaceholder: 'Cari potential, produk, ekonomi, resource...',
    emptyDescription: 'Belum ada potential. Tambahkan data potensi untuk digunakan pada website dan section public.',
    brand: '#58cc02',
    brandSoft: '#f0fdf4',
...
const {
  ...
  error,
  refresh
} = useFetch<PotentialListResponse>(apiUrl, {
  key: computed(() => `app-potentials-${tenantSlug.value}-${selectedStatus.value}-${selectedType.value}-${sortBy.value}`),
  query: computed(() => ({
...
const potentials = computed<PotentialItem[]>(() => potentialsResponse.value?.data || [])
const isLoading = computed(() => Boolean(unref(pending)))
const visibleError = computed(() => {
  const value = unref(error)
  if (!value) return ''
  if (typeof value === 'string') return value
  return value?.message || 'Terjadi kesalahan saat memuat potensi.'
})
...
const typeOptions: { label: string; value: PotentialType; icon: string }[] = [
  { label: 'Pertanian', value: 'agriculture', icon: 'lucide:wheat' },
  { label: 'Peternakan', value: 'livestock', icon: 'lucide:cow' },
  { label: 'Perikanan', value: 'fishery', icon: 'lucide:fish' },
  { label: 'UMKM', value: 'umkm', icon: 'lucide:store' },
  { label: 'Wisata', value: 'tourism', icon: 'lucide:map-pin' },
  { label: 'Budaya', value: 'culture', icon: 'lucide:landmark' },
  { label: 'Sumber Daya Alam', value: 'natural_resource', icon: 'lucide:mountain' },
  { label: 'Sumber Daya Manusia', value: 'human_resource', icon: 'lucide:users' },
  { label: 'Industri', value: 'industry', icon: 'lucide:factory' },
  { label: 'Ekonomi Kreatif', value: 'creative_economy', icon: 'lucide:palette' },
  { label: 'Kuliner', value: 'food', icon: 'lucide:utensils' },
  { label: 'Kerajinan', value: 'craft', icon: 'lucide:gem' },
  { label: 'Custom', value: 'custom', icon: 'lucide:layers' }
]

const formSteps = [
  { value: 1, title: 'Identitas', description: 'Nama & tipe', icon: 'solar:leaf-bold-duotone' },
  { value: 2, title: 'Media', description: 'Logo & gambar', icon: 'solar:gallery-wide-bold-duotone' },
  { value: 3, title: 'Konten', description: 'Produksi & kontak', icon: 'solar:pen-new-square-bold-duotone' },
  { value: 4, title: 'Review', description: 'Cek & simpan', icon: 'solar:clipboard-check-bold-duotone' }
]

const form = reactive({
  potentialType: 'custom' as PotentialType,
  title: '',
  subtitle: '',
  slug: '',
  icon: '' as string | null,
  ...
  marketReach: '',
  estimatedValue: '',
  status: 'active' as PotentialStatus,
  isFeatured: true,
  sortOrder: 0,
...
const stats = computed(() => {
  const rows = potentials.value
  const active = rows.filter((item) => item.status === 'active').length
  const featured = rows.filter((item) => item.isFeatured).length
  const withImages = rows.filter((item) => getPotentialImages(item).length).length
  const categories = new Set(rows.map((item) => item.potentialType)).size

  return [
    { label: 'Total Potensi', value: String(rows.length), icon: 'solar:leaf-bold-duotone', description: 'Total potensi pada tenant ini.' },
    { label: 'Active', value: String(active), icon: 'solar:check-circle-bold-duotone', description: 'Potensi aktif yang bisa ditampilkan.' },
    { label: 'Featured', value: String(featured), icon: 'solar:star-fall-bold-duotone', description: 'Diprioritaskan pada section public.' },
    { label: 'Kategori', value: String(categories || withImages), icon: 'solar:widget-5-bold-duotone', description: 'Variasi tipe potensi tenant.' }
  ]
})
...
function ensureEditor() {
  ...
  editor.value = new Editor({
    ...
    extensions: [
      ...
      Link.configure({
        openOnClick: false,
        HTMLAttributes: { class: 'text-emerald-600 underline underline-offset-4' }
      }),
      Placeholder.configure({ placeholder: 'Tulis deskripsi lengkap potensi di sini...' }),
      Image.configure({
        inline: false,
        allowBase64: false,
...
async function reloadPotentials() {
  try {
    ...
    showToast('error', 'Gagal Memuat Potensi', error?.message || 'Data potensi gagal dimuat.')
  }
}
...
function openEdit(item: PotentialItem) {
  ...
  form.potentialType = item.potentialType || 'custom'
  form.title = item.title || ''
  form.subtitle = item.subtitle || ''
  form.slug = item.slug || ''
  form.icon = item.icon || defaultIconByType(item.potentialType || 'custom')
  ...
  form.productionCapacity = item.productionCapacity || ''
  form.productionUnit = item.productionUnit || ''
  form.marketReach = item.marketReach || ''
  form.estimatedValue = item.estimatedValue === null || typeof item.estimatedValue === 'undefined' ? '' : String(item.estimatedValue)
  form.status = item.status || 'active'
  form.isFeatured = Boolean(item.isFeatured)
  form.sortOrder = Number(item.sortOrder || 0)

  for (const image of form.images) revokeObjectURL(image.preview)
  form.images = getPotentialImages(item).map((url) => ({
    id: createLocalId(),
    url,
    preview: url,
    file: null,
...
function validateCurrentFormStep() {
  ...
  if (formStep.value === 1) {
    if (!form.title.trim()) {
      formError.value = 'Nama potensi wajib diisi.'
      return false
    }
    ...
    if (!form.slug) {
      formError.value = 'Slug potensi tidak valid.'
      return false
    }
  ...
  if (formStep.value === 3) {
    ...
    if (form.estimatedValue && Number.isNaN(Number(form.estimatedValue))) {
      formError.value = 'Estimasi nilai harus berupa angka.'
      return false
    }
...
async function submitForm() {
  ...
  try {
    ...
    const previousMetadata = selectedPotential.value?.metadata || {}

    const payload: CreatePotentialPayload | UpdatePotentialPayload = {
      potentialType: form.potentialType,
      title: form.title.trim(),
      subtitle: form.subtitle.trim() || null,
      slug: slugify(form.slug || form.title),
      icon: form.icon || defaultIconByType(form.potentialType),
    ...
    if (formMode.value === 'create') {
      ...
      showToast('success', 'Potensi Berhasil Dibuat', 'Data potensi berhasil disimpan.')
    } else if (selectedPotential.value) {
      await $fetch(`${apiUrl.value}/${selectedPotential.value.id}`, {
        method: 'PUT',
        body: payload
      })

      showToast('success', 'Potensi Berhasil Diupdate', 'Perubahan potensi berhasil disimpan.')
    }

    showForm.value = false
    await reloadPotentials()
  } catch (error: any) {
    const message = error?.data?.statusMessage || error?.data?.message || error?.statusMessage || error?.message || 'Gagal menyimpan potensi.'
...
async function confirmDelete() {
  ...
  try {
    ...
    showToast('error', 'Gagal Menghapus', error?.message || 'Data potensi gagal dihapus.')
  } finally {
...
function getPotentialCover(item: PotentialItem) {
  return getPotentialImages(item)[0] || item.logoUrl || ''
}
...
function potentialTypeLabel(type: PotentialType) {
  return typeOptions.find((item) => item.value === type)?.label || 'Potensi'
}

function defaultIconByType(type: PotentialType) {
...
async function onPickEditorImage(event: Event) {
  ...
  try {
    const result = await uploadImage(file, {
      folder: `${tenantSlug.value}/potentials/content`
    })
...
async function searchIconifyIcons() {
  const query = iconQuery.value.trim() || 'wheat'
