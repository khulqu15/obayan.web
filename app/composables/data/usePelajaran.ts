...
import { ref as dref, push, set, update, remove, get, onValue, off, serverTimestamp, query as dquery, orderByChild } from 'firebase/database'
...
export function usePelajaran() {
  ...
  function subscribeSubjects() {
    ...
    const base = dref($realtimeDb, `${clientName}/pelajaran`)
  ...
  async function createSubject(p: Omit<MapelItem,'id'|'createdAt'|'updatedAt'>){
    const node = push(dref($realtimeDb, `${clientName}/pelajaran`))
  ...
  async function updateSubject(id:string, patch: Partial<Omit<MapelItem,'id'>>){
    const node = dref($realtimeDb, `${clientName}/pelajaran/${id}`)
  ...
  async function deleteSubject(id:string){ await remove(dref($realtimeDb, `${clientName}/pelajaran/${id}`)) }
  ...
  function subscribeNilai(termKey:string, mapelId:string){
    ...
    const node = dref($realtimeDb, `${clientName}/nilai/${termKey}/${mapelId}`)
  ...
  async function readWeights(termKey:string, mapelId:string):Promise<GradeWeights>{
    try{ const s=await get(dref($realtimeDb, `${clientName}/nilai_weights/${termKey}/${mapelId}`)); const v=s.val()||{}; return { tugas:Number(v.tugas??DEFAULT_WEIGHTS.tugas), harian:Number(v.harian??DEFAULT_WEIGHTS.harian), pts:Number(v.pts??DEFAULT_WEIGHTS.pts), pas:Number(v.pas??DEFAULT_WEIGHTS.pas), proyek: v.proyek!==undefined?Number(v.proyek):undefined, lainnya: v.lainnya!==undefined?Number(v.lainnya):undefined } }
    catch { return DEFAULT_WEIGHTS }
  }
  async function saveWeights(termKey:string, mapelId:string, w:GradeWeights){
    await update(dref($realtimeDb, `${clientName}/nilai_weights/${termKey}/${mapelId}`), { tugas:Number(w.tugas||0), harian:Number(w.harian||0), pts:Number(w.pts||0), pas:Number(w.pas||0), proyek:w.proyek!==undefined?Number(w.proyek):null, lainnya:w.lainnya!==undefined?Number(w.lainnya):null, updatedAt:serverTimestamp() })
  }
  ...
  async function saveNilai(termKey:string, mapelId:string, santriId:string, patch:Partial<GradeRecord>){
    ...
    await update(dref($realtimeDb, `${clientName}/nilai/${termKey}/${mapelId}/${santriId}`), {
  ...
  async function bulkSaveNilai(termKey:string, mapelId:string, payload:Record<string,GradeRecord>){
    const sub = getSubject(mapelId)
    const w = await readWeights(termKey, mapelId)
    const base = dref($realtimeDb, `${clientName}/nilai/${termKey}/${mapelId}`)
