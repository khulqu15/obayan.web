...
export type SantriRow = {
  id: string; gen: string; santri: string; walisantri?: string; nohp?: string; kuotaKunjunganBulanIni?: number;
  kamar?: string; alamat?: string; status?: string; jenjang?: string; maskan?: string; rfid?: any; fingerprint?: any
}

export const useSantri = () => {
  const loading = vRef(false); const error = vRef<string | null>(null)
  const rows = vRef<SantriRow[]>([]); let _unsubscribe: (() => void) | null = null
  ...
  async function fetchSantri() {
    ...
    try {
      ...
      rows.value = Object.entries(val).map(([key, v]: any) => ({
        id: key, gen: v.gen || '', santri: v.santri || v.nama || '', walisantri: v.walisantri || v.wali || v.ortu || '',
        nohp: v.nohp || v.no_wa || v.whatsapp || '', kuotaKunjunganBulanIni: Number(v.kuotaKunjunganBulanIni ?? v.kuota_bulan_ini ?? 0),
        kamar: v.kamar || v.asrama || '', maskan: v.maskan || '', status: v.status || '', alamat: v.alamat || '', jenjang: v.jenjang || v.jenjang_pendidikan || v.kelas || '',
        rfid: v.rfid || '', fingerprint: v.fingerprint || '',
      }))
    } catch (e:any) { console.error(e); error.value = e?.message ?? 'Gagal memuat data' }
  ...
  async function createSantri(payload: Omit<SantriRow, 'id'>, opts: { refresh?: boolean } = {}) {
    ...
    const data = {
      ...
      kuotaKunjunganBulanIni: Number(payload.kuotaKunjunganBulanIni ?? 0), kamar: payload.kamar ?? '', maskan: payload.maskan ?? '', alamat: payload.alamat ?? '',
      status: payload.status ?? '', jenjang: payload.jenjang ?? '', rfid: payload.rfid || '', fingerprint: payload.fingerprint || '',
    }
    await set(newRef, data); if (refresh) await fetchSantri(); return newRef.key
  }

  async function updateSantri(id: string, payload: Partial<Omit<SantriRow, 'id'>>, opts: { refresh?: boolean } = {}) {
    const { refresh = true } = opts; const { $realtimeDb } = useNuxtApp(); const nodeRef = dbRef($realtimeDb, `alberr/santri/${id}`)
    const data:any = {}
    ...
    await update(nodeRef, data); if (refresh) await fetchSantri()
  }
        ...
        chunk: async (results: any) => {
          ...
          for (const row of data) {
            await createSantri({
              ...
              kamar: String(pick(row, ['Kamar','Asrama','kamar'], '')), maskan: String(pick(row, ['Maskan'], '')),
              alamat: String(pick(row, ['Alamat ','Alamat',' Alamat'], '')), status: String(pick(row, ['Status'], '')),
              jenjang: String(pick(row, ['Jenjang','Kelas','Kelas Formal','jenjang'], '')),
              rfid: String(pick(row, ['RFID','Rfid','rfid'], '')), fingerprint: String(pick(row, ['Fingerprint','fingerprint','FINGERPRINT'], '')),
            }, { refresh: false })
  ...
  async function deleteSantri(id: string) {
    const { $realtimeDb } = useNuxtApp()
    await remove(dbRef($realtimeDb, `alberr/santri/${id}`)); await fetchSantri()
  }
  ...
  return { rows, loading, error, fetchSantri, createSantri, updateSantri, deleteSantri, importFromExcelWithProgress, subscribeSantri, unsubscribeSantri }
}
