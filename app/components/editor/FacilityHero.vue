  ...
  <div>
    ...
    <div v-show="activeTab==='Stats'" class="mt-3 space-y-3">
        ...
        <div class="grid sm:grid-cols-3 gap-3 items-center">
          ...
          <div>
            ...
            <div class="mt-1 flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                class="px-2 py-1 text-xs rounded border border-gray-200 hover:bg-gray-50 dark:border-neutral-700 dark:hover:bg-neutral-800"
      ...
      <div class="flex gap-2">
        <button
          class="px-2 py-1 text-xs rounded border border-gray-200 hover:bg-gray-50 dark:border-neutral-700 dark:hover:bg-neutral-800"
          @click="form.stats.push({label:'Label', value:'0', icon:'ph:info'})"
        >
          Tambah Stat
        </button>
        <button
          v-if="form.stats.length>0"
          class="px-2 py-1 text-xs rounded border border-gray-200 text-rose-600 hover:bg-rose-50 dark:border-neutral-700 dark:hover:bg-neutral-800"
...
const props = defineProps<{
  ...
  pagePath?: string
  /** Penting: pass instance useWeb() dari parent supaya sinkron */
  web?: ReturnType<typeof useWeb> | any
}>()

/** Pakai instance web dari parent kalau ada; kalau tidak, fallback ke useWeb() (standalone) */
const web = props.web || useWeb()
const { updateSection, uploadMedia } = web

/** Saat pagePath tersedia/berubah, set active path di instance yang sama */
watch(() => props.pagePath, (p) => {
  if (p && (web as any)?.setActivePath) (web as any).setActivePath(p)
}, { immediate: true })
...
/** Form = defaults ⊕ props.section.props */
const form = reactive<Shape>(merge(defaults, props.section?.props || {}))

/** Ketika props.section.props berubah dari server, sinkronkan form */
watch(
  () => props.section?.props,
  (p) => { Object.assign(form, merge(defaults, p || {})) },
  { deep: false }
)
...
/* ===== Upload pakai uploadMedia dari useWeb instance yang sama ===== */
