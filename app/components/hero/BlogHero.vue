      ...
      <div class="grid grid-cols-12 gap-6 mt-8">
        ...
        <div class="lg:col-span-6 col-span-12">
          <div class="card-blog max-h-[34rem] min-h-[34rem] overflow-hidden relative h-full w-full border rounded-xl border-gray-200 dark:border-neutral-700 bg-white/70 dark:bg-neutral-800/60 backdrop-blur-sm hover:shadow-md transition-shadow">
            <img class="card-blog-image transition-all" :src="state.featured.cover" alt="">
            <div class="card-blog-content p-5 overflow-hidden">
              <span class="inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-lg text-xs font-medium bg-blue-100 dark:bg-blue-800/30 mb-3">{{ state.featured.category }}</span>
              <h1 class="text-xl font-semibold text-gray-800 dark:text-gray-100">
                {{ state.featured.title }}
              </h1>
              <div class="flex justify-between items-center flex-wrap mt-3">
                <div class="flex-grow flex gap-1">
                  <span v-for="t in (state.featured.tags || [])" :key="t"
                ...
                <div class="flex-grow md:w-auto w-full text-right">
                  <span class="text-sm text-gray-800 dark:text-white">{{ state.featured.dateText }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="col-span-12 lg:col-span-6 space-y-4">
          ...
          <div class="grid sm:grid-cols-2 gap-3">
            <a v-for="post in miniList" :key="post.id" :href="post.href"
        ...
        <NuxtLink href="/news" class="text-right flex justify-end col-span-12">
...
const defaults: Shape = {
  ...
  posts: [
    ...
    { id:'p-05', title:'Manajemen Asrama: Disiplin & Kemandirian', href:'#', excerpt:'Checklist harian asrama...', cover:'/assets/images/activity1.jpg', dateText:'25 Juli 2025', readTime:4, category:'Kepesantrenan', tags:['asrama','disiplin'] },
    { id:'p-06', title:'Perpustakaan Aktif & Literasi Santri', href:'#', excerpt:'Strategi kurasi koleksi...', cover:'https://images.unsplash.com/photo-1519681393784-d120267933ba?q=80&w=1200&auto=format&fit=crop', dateText:'22 Juli 2025', readTime:6, category:'Sumber Daya', tags:['perpustakaan','literasi'] },
    { id:'p-07', title:'Olahraga Santri: Kurikulum Kebugaran', href:'#', excerpt:'Template mingguan latihan fisik...', cover:'/assets/images/activity2.jpg', dateText:'18 Juli 2025', readTime:3, category:'Ekstrakurikuler', tags:['olahraga','kebugaran'] },
    { id:'p-08', title:'Zakat & Donasi Digital: QRIS & VA', href:'#', excerpt:'Alur dan pelaporan ringkas donasi...', cover:'https://images.unsplash.com/photo-1604594849809-dfedbc827105?q=80&w=1200&auto=format&fit=crop', dateText:'15 Juli 2025', readTime:5, category:'Keuangan', tags:['qris','donasi'] },
  ],
}
...
const categoriesUi = computed(() => ['Semua', ...Array.from(new Set(state.posts.map(p => p.category))).sort()])
const tagsUi = computed(() => Array.from(new Set(state.posts.flatMap(p => p.tags))).sort((a,b)=>a.localeCompare(b)))
...
const filtered = computed(() => {
  ...
  return state.posts.filter(p => catOk(p) && tagsOk(p))
})
...
onMounted(() => { initCarousel(); fetchFromFirebase() })
...
function merge(base: Shape, patch: Partial<Shape> = {}): Shape {
  return {
    ...
    posts: Array.isArray(patch.posts) && patch.posts.length ? patch.posts as Post[] : base.posts
  }
}
async function fetchFromFirebase() {
  ...
  try {
    ...
    if (Array.isArray(data.sections)) {
      const sec = data.sections.find((s: any) => s?.key === 'BlogHero' || s?.key === 'NewsHero')
      props = sec?.props
    }
    if (!props && data.blogHero) props = data.blogHero
    if (!props && data.newsHero) props = data.newsHero
